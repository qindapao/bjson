...
_bjson_quote ()
{
    ...
    bjsonQuoteR_str=${bjsonQuoteR_str//$'\b'/\\b} # 替换退格符（可选）
    bjsonQuoteR_str=${bjsonQuoteR_str//$'\a'/\\a} # 替换响铃符（可选）
    bjsonQuoteR_str=${bjsonQuoteR_str//$'\f'/\\f} # 替换翻页符（可选）
    # 嵌入 HTML 时才需要转义
...
bjson_map_to_json () 
{
    ...
    for bjsonMapToJsonV_k in "${!bjsonMapToJsonR_map[@]}"; do
        ...
        ((bjsonMapToJsonV_isValueJson)) || _bjson_quote bjsonMapToJsonV_v
...
bjson_arr_to_json ()
{
    ...
    local bjsonArrToJsonV_isItemJson="${3:-0}"
    ...
    for bjsonArrToJsonV_item in "${bjsonArrToJsonR_arr[@]}"; do
        ((bjsonArrToJsonV_isItemJson)) || _bjson_quote bjsonArrToJsonV_item
...
bjson_w_map_to_jstr ()
{
    ...
    bjson_map_to_json "$2" bjsonWMapToJStrV_mapjStr "$3"
    bjson_w_jobj_to_jstr "$1" "$bjsonWMapToJStrV_mapjStr" "${@:4}"
}
...
bjson_w_map_to_jfile ()
{
    ...
    bjson_map_to_json "$2" bjsonWMapToJfileV_mapjStr "$3"
    bjson_w_jobj_to_jfile "$1" "$bjsonWMapToJfileV_mapjStr" "${@:4}"
}
...
bjson_w_arr_to_jstr ()
{
    ...
    bjson_arr_to_json "$2" bjsonWArrToJStrV_mapjStr "$3"
    bjson_w_jobj_to_jstr "$1" "$bjsonWArrToJStrV_mapjStr" "${@:4}"
}
...
bjson_w_arr_to_jfile ()
{
    ...
    bjson_arr_to_json "$2" bjsonWArrToJfileV_mapjStr "$3"
    bjson_w_jobj_to_jfile "$1" "$bjsonWArrToJfileV_mapjStr" "${@:4}"
}
...
{
    gobolt json -m r -t sh -k file -i $1 -v $2 -P -- "${@:3}"
}
...
bjson_r_to_var_fstr ()
{
    printf "%s" "$1" | gobolt json -m r -t sh -k stdin -v $2 -P -- "${@:3}"
}
...
{
    gobolt json -l "/dev/null" -m r -t type -k file -i "$1" -P -- "${@:2}"
}
...
bjson_get_attr_fstr ()
{
    printf "%s" "$1" | gobolt json -l "/dev/null" -m r -t type -k stdin -P -- "${@:2}"
}
